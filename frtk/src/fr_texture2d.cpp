...
bool Fr_Texture2D::set2DTexture(std::string& path, int width, int height, int bpp, int compno) {
    m_FilePath = path;
    m_width = width;
    m_height = height;
    m_BPP = bpp;
...
bool Fr_Texture2D::setup2DTexture() {
    ...
    glCheckFunc(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_width, m_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_LocalBuffer));
